            ...
            <div class="col-md-8">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('cities.update', $city->id) }}">
                            ...
                            <div class="form-group row">
                                <label for="estado_id" class="col-md-4 col-form-label text-md-right">{{ __('Cidade') }}</label>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="estado_id" class="form-control" aria-label="Default select example">
                                        ...
                                        <option value="{{ $state->id }}" 
                                                {{$state->id == $city->estado_id ? 'selected' : ''}}> 
                                                {{$state->name}}</option>
                                        @endforeach
                ...
                <div class="m-2 p-2">
                    <form method="POST" action="{{ route('cities.destroy', $city->id) }}">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-danger">Excluir a cidade: {{ $city->name }}</button>
                    </form>
                </div>
            </div>
        </div>
